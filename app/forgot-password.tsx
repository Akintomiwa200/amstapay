...
export default function ForgotPasswordScreen() {
  ...
  const handleRequestReset = async () => {
    ...
    try {
      ...
      const response = await fetch("https://amstapay-backend.onrender.com/api/v1/auth/forgot-password", {
        method: "POST",
      ...
      // In production, the API sends an email with a token
      // For now, we'll show the token if provided (dev mode)
      if (data.token) {
        setToken(data.token);
        setStep("reset");
      }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      // Call the real API endpoint
      const response = await fetch("https://amstapay-backend.onrender.com/api/v1/auth/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: email.trim(),
        ...
        {step === "request" && (
          <>
            ...
            {successMsg && (
              ...
              >
                <Text style={s.nextStepButtonText}>I have a reset token</Text>
              </TouchableOpacity>
            )}
          </>
        )}
